...
function gaugeChart(dom) {
  var margin = {top: 0, right: 65, bottom: 10, left: 65},
      ...
      innerRadius = 60,
      outerRadius = 80,
      dataDomain = dom, /* Dominio de datos */
      labelPad = 10,
...
window.onload = function(){
     ...
     function resize() {
        ...
        gauge.width(gWidth).innerRadius(gWidth / 4).outerRadius((gWidth / 4) + 40);
         gaugeHum.width(gWidth).innerRadius(gWidth / 4).outerRadius((gWidth / 4) + 40);
        d3.select("#chart").call(gauge);
        d3.select("#chartHum").call(gaugeHum);
      }

    
    d3.select("#chart").datum([0]).call(gauge);
     d3.select("#chartHum").datum([0]).call(gaugeHum);
    ...
    window.addEventListener("resize", resize);
      ...
      var gauge = gaugeChart( [0, 15, 30])
                    .width(260)
                    .height(200)
                    .innerRadius(50)
                    .outerRadius(80);

      var gaugeHum = gaugeChart( [0, 50, 100])
                    .width(260)
                    .height(200)
                    .innerRadius(50)
                    .outerRadius(80);
